...
void printArray(const std::array<std::array<double, COLS>, ROWS>& arr) {
    for (size_t i = 0; i < 10; ++i) { // Печатаем только первые 10 строк для краткости
        for (size_t j = 0; j < 10; ++j) { // Печатаем только первые 10 элементов в строке
            std::cout << arr[i][j] << " ";
        }
...
int main() {
    ...
    try {
        loadArrayFromFile("matrix.txt", matrix1);
